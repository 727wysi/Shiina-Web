...
public class ClanQuery {
    ...
    private final String GETCLAN_ACTIVITY = "SELECT s.`userid` AS `user_id`, u.`name`, m.`filename`, m.`id` AS `map_id`, m.`set_id`, s.`play_time`, s.`mods`, s.`pp`, s.`id` AS `score_id`, s.`acc`, s.`grade` FROM `scores` s INNER JOIN `users` u ON s.`userid` = u.`id` INNER JOIN `maps` m ON m.`md5` = s.`map_md5` INNER JOIN ( SELECT `userid`, `map_md5`, `mods`, MAX(`play_time`) AS `max_play_time` FROM `scores` WHERE `status` = 2 AND `mode` = ? GROUP BY `userid`, `map_md5`, `mods` ) latest ON s.`userid` = latest.`userid` AND s.`map_md5` = latest.`map_md5` AND s.`mods` = latest.`mods` AND s.`play_time` = latest.`max_play_time` WHERE u.`clan_id` = ? AND s.`status` = 2 AND s.`mode` = ? ORDER BY s.`play_time` DESC LIMIT 20;";
